...
| `riderVisibilityReason` | String \| null | Why the current rider visibility decision was made, for example `gps-clear-required`, `current-detour-vehicle`, `insufficient-geometry`, `suppressed-invalid-geometry`, `stale-mixed-evidence`, `stale-sparse-evidence`, or `zero-confirmed-vehicle-count`. |
...
Geometryless or unsafe detours are backend-only. If the detector has an active record but cannot publish trustworthy rider geometry, the backend keeps monitoring and writes `riderVisible: false`, usually with `riderVisibilityReason: "insufficient-geometry"`, `"stale-mixed-evidence"`, or `"suppressed-invalid-geometry"`. The client must not show a detour alert, route detour card, or overlay for that record. If later GPS evidence creates a trusted skipped segment or likely path, the backend can make it rider-visible. Once a configured, road-matched, or sufficiently dense GPS path has become rider-visible, a between-bus snapshot with no newer evidence keeps that trusted path visible with `gps-clear-required`; stale mixed evidence may block an unsafe initial path or a materially weaker replacement, but it must not make a trusted path flash on only while a bus is inside the corridor. If normal service is proven, the backend can clear it.
...
- **Rider visibility** — confirmed detours stay rider-visible until GPS clear proof exists. The app must not hide a detour from age, `staleForReview`, no-current-vehicle state, or route-family activity. Initial geometry that is insufficient, invalid, stale-mixed, or Route 400 stale-sparse remains backend-only. After a configured, road-matched, or sufficiently dense GPS path is public, between-bus samples keep its last trusted geometry visible instead of alternating between `current-detour-vehicle` and `stale-mixed-evidence`; only new contradictory/unsafe geometry or clear proof may revoke it. Weak or jumpy stale-mixed paths remain hidden until current same-corridor evidence reconfirms a trustworthy path.
...
- **Path-confidence gate** — geometry-backed detour alerts can remain active while the map hides the alternate detour path until the backend sees trusted path evidence: either same-bus before/off-route/after evidence or two distinct buses with entry/exit anchors on the same corridor. When `canShowDetourPath=true`, the app can draw the trusted inferred path even before road matching produces a `likelyDetourPolyline`. If a segment has `canShowDetourPath=false`, that segment's alternate path is hidden and the client must not use stale top-level geometry as a fallback. If there is no trustworthy skipped segment or path, the whole detour remains backend-only.
...
- **Safe inferred-path handoffs** — when a trusted inferred GPS path starts or ends a short distance from its regular-route boundary, V2 extends the rider line to that boundary. The default maximum is 150m. Larger gaps are left alone rather than drawing a misleading straight connection across blocks.
...
| — | — | Except for the obsolete-shape fallback above, there is intentionally no time-based clear or rider hide rule for active detector-owned detours. They are retained and, when confirmed with safe geometry, remain visible so later buses can add evidence or prove normal routing. |
