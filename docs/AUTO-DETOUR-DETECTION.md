...
| `riderVisible` | Boolean | Whether the client should show this active backend detour to riders. Stale or zero-confirmed-evidence records can stay in Firestore for audit while being hidden from rider UI. |
| `riderVisibilityReason` | String \| null | Why the current rider visibility decision was made, for example `fresh-enough`, `current-detour-vehicle`, `stale-evidence-gps-clear-required`, or `zero-confirmed-vehicle-count`. |
| `staleForReview` | Boolean | Whether the detour should be reviewed by operations because it is no longer rider-visible or is stale enough to need attention. |
| `staleAgeMs` / `staleThresholdMs` | Number \| null | Evidence age and the headway-aware limit used for rider visibility. |
| `scheduledHeadwayMs` / `scheduleSource` | Number/String \| null | Schedule context used to compute the stale visibility threshold. |
...
Self-loop segments are not valid closures when they enter and exit at the same stop and only that stop is affected. Segments with no entry stop, no exit stop, no skipped route segment, and explicit empty stop-impact fields are also not valid rider-facing closures. The backend filters both cases before publishing and before preserving a previously trusted likely path.
...
- **Headway-aware rider visibility** — stale active backend records can stay in Firestore while being hidden from rider UI when no route-family vehicle is currently reporting and there is no fresh evidence beyond `max(45min, 2 × headway + 10min)`, capped at 3 hours. Zero-confirmed-vehicle active records are also hidden from riders.
