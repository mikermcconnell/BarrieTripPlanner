...
- **Baseline divergence rider suppression** — if a route's stored detour baseline meaningfully differs from the current public GTFS geometry, active records for that route stay in Firestore for monitoring and GPS-clear handling, but are hidden from riders with `riderVisibilityReason: "baseline-diverged"` until a detour admin approves a route baseline refresh.
...
| `DETOUR_CLEAR_WINDOW_MIN_COVERAGE_RATIO` | `0.95` | Minimum share of the clear window that must be covered by same-bus traversal or collective interval coverage before clearing. |
