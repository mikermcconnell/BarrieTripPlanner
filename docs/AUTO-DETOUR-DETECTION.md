...
| `riderVisibilityReason` | String \| null | Why the current rider visibility decision was made, for example `gps-clear-required`, `current-detour-vehicle`, `insufficient-geometry`, `suppressed-invalid-geometry`, `stale-sparse-evidence`, or `zero-confirmed-vehicle-count`. |
