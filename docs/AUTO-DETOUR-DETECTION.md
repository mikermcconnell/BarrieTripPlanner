...
| `entryConnectorPolyline` / `exitConnectorPolyline` | String \| null | Optional road-matched transition connectors that fill the small approach/rejoin gaps created when normal-route endpoint overlap is trimmed from `likelyDetourPolyline`. The client stitches these onto the renderable detour path but keeps the core likely path available separately for road names/confidence. |
...
Key `segments[]` fields mirror the top-level geometry and stop-impact fields: `shapeId`, `skippedSegmentPolyline`, `inferredDetourPolyline`, `likelyDetourPolyline`, `entryConnectorPolyline`, `exitConnectorPolyline`, `canShowDetourPath`, `entryPoint`, `exitPoint`, `entryStopId`, `exitStopId`, `skippedStops`, `affectedStops`, `clearWindow`, `confidence`, `evidencePointCount`, `lastEvidenceAt`, `detourEventId`, and shared-event metadata.
...
- **Updated 2026-06-11: road-matched detour paths can include transition connectors** — When road matching trims normal-route overlap at the start or end of a likely path, the backend now publishes entry/exit connector polylines and the client stitches them into the rendered detour path. This closes visual gaps caused by conservative off-route thresholds without lowering detection thresholds.
