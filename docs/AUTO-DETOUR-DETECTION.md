...
| `alertVisible` | Boolean | Whether a confirmed active detour alert should be public. This is independent of path and stop confidence. Once two valid identities confirm an active detour, the alert stays public until GPS clear proof, an operator clear, or a detection-integrity block such as baseline divergence or invalid self-loop geometry. |
| `alertVisibilityReason` | String \| null | Why the alert is public or hidden. `active-detour-details-unavailable` means the active alert is public while unsafe path and stop details remain hidden. New clients use this field when present and fall back to `riderVisible` for older records. |
...
| `likelyDetourPolyline` | String \| null | Optional road-matched path shown to riders as the likely detour path. For newly matched geometry, this is the final rider-renderable line and includes any safe normal-route approach/rejoin transition needed for a continuous visual handoff from the regular route. |
| `entryConnectorPolyline` / `exitConnectorPolyline` | String \| null | Legacy transition connectors from earlier road-matched records. New road-matched records stitch these transitions into `likelyDetourPolyline` before publishing and set connector fields to `null` to avoid client-side double stitching. |
...
Confirmed active detour alerts and detailed geometry use separate gates. If the detector cannot publish trustworthy rider geometry, the backend keeps `alertVisible: true` but writes `riderVisible: false` and `canShowDetourPath: false`, usually with `alertVisibilityReason: "active-detour-details-unavailable"` and a geometry reason such as `"insufficient-geometry"` or `"stale-mixed-evidence"`. The client shows a plain active-detour alert while withholding the exact path and uncertain stop impacts. A newly confirmed event remains alert-visible between buses, and one new matching bus refreshes its heartbeat. If the exact route keeps reporting but no matching GPS evidence refreshes the event for 90 minutes, rich geometry may be suppressed for safety, but the confirmed alert remains public until normal-route GPS traversal or explicit operator action resolves it. Baseline divergence, invalid self-loop evidence, zero confirmed identities, and cleared records remain fully hidden.
...
One document per event mutation. Deterministic IDs use `{timestamp}-{eventId}-{eventType}` so retrying the same mutation is idempotent. Active-state and history writes/deletes are committed together in one Firestore batch.
...
- **Alert visibility** — a confirmed active alert stays public until affected-segment GPS traversal or operator action clears it. `staleForReview`, no-current-vehicle state, elapsed time, stale-mixed geometry, and route-family activity do not hide the alert. Baseline divergence, invalid self-loop evidence, low/insufficient confirmation, and cleared records remain hidden.
- **Detail visibility** — path and stop details fail closed independently. Unsafe, stale-mixed, disconnected, or insufficient geometry may set `riderVisible: false`, `canShowDetourPath: false`, and remove uncertain stops while `alertVisible` remains true. One new matching bus refreshes the confirmed event heartbeat without re-running initial two-identity confirmation.
...
- **Path-confidence gate** — confirmed alerts remain active while the map hides the alternate detour path until the backend sees trusted entry/exit and corridor evidence. When `canShowDetourPath=true`, the app can draw the trusted inferred path even before road matching produces a `likelyDetourPolyline`. If a segment has `canShowDetourPath=false`, that segment's alternate path is hidden and the client must not use stale top-level geometry as a fallback. If no trustworthy path or skipped segment exists, the alert remains available without map or stop details.
...
- **Updated 2026-08-18: normal Downtown Hub egress is matched by terminal and path, not route ID** — Some scheduled shapes continue directly onto Maple while buses normally leave Downtown Hub Stop `2` through the Simcoe/Mary/Dunlop street loop. GTFS trip metadata now includes first/last stop IDs, and the V2 detector classifies a point as `known-terminal-circulation` only when the trip begins at Stop `2`, route progress remains within the first 250m, and GPS stays within 55m of the operator-confirmed circulation polyline. This applies to Route `8A`, Route `100`, and future routes using the same movement without creating a blanket downtown exclusion; a materially different terminal-edge path still contributes detour evidence.
...
- **Corrected 2026-07-15: alert status is independent from geometry confidence** — `staleForReview`, stale-mixed geometry, and the 90-minute rich-detail ceiling may hide an unsafe path or uncertain stop impacts, but they no longer hide a confirmed active alert. Only GPS clear proof, operator clear, or a detection-integrity block removes the public alert.
...
| `DETOUR_RIDER_VISIBILITY_MAX_EVIDENCE_AGE_MS` | `5400000` | Rich-detail visibility ceiling only. When the exact route is still reporting but an event has no matching GPS evidence for 90 minutes, hide stale geometry and uncertain stop details. The confirmed alert remains public and the active record still requires GPS clear proof or operator action. |
...
### Detector Storage Selection
| Variable | Default | Description |
|---|---|---|
| `DETOUR_DETECTOR_VERSION` | `v2` | Backend storage selector. V2 uses `activeDetourEventsV2`, `detourEventHistoryV2`, and `systemState/detourRuntimeV2`. Set explicit `v1` only for the legacy route-scoped collections; invalid values fail startup. |
...
| **GTFS-RT feed frozen, untimestamped, or future-dated** | Raw positioned entities are still counted in `vehicleFeed`, but samples without valid timestamps, older than 5 minutes, or more than the configured future-skew allowance ahead are filtered before detection. `/api/detour-status` reports `stale`, `unknown`, or `future`; untrustworthy GPS does not create or clear detours. |
...
| **Firestore write fails** | Active state and its history event share one Firestore batch, so neither side commits alone. The deterministic history ID makes a retry idempotent. The error is logged and the publisher retries next tick. |
...
- `vehicleFeed` — raw GTFS-RT vehicle feed health, including raw entity count, positioned/usable counts, total filtered count, missing/future timestamp counts, and freshness status
