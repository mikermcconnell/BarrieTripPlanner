...
| `riderVisible` | Boolean | Whether the client should show this active backend detour to riders. Confirmed detours remain stored until normal-route GPS clear proof exists. Rider visibility is separately suppressed for unsafe geometry and when the exact route continues reporting but the event has no fresh GPS evidence for the configured 90-minute visibility window. |
...
| `alertVisible` | Boolean | Whether an active backend record is eligible to become a public rider alert. The record remains stored until GPS clear proof, but public auto-detour eligibility requires current confirmed GPS evidence with meaningful bounded geometry. Official notices may enrich an already-qualified auto-detour but never create or preserve one. |
| `alertVisibilityReason` | String \| null | Why the alert is public or hidden. Current reasons include `fresh-confirmed-gps-evidence`, `persistent-operator-detour`, `insufficient-alert-geometry`, `insufficient-recent-gps-evidence`, and `stale-unresolved-awaiting-gps-clear`. New clients use this field when present and fall back to `riderVisible` for older records. |
...
- **Updated 2026-07-15: stale visibility is separate from GPS clearing** — Elapsed time still never clears an active detour. To prevent old alerts from remaining public indefinitely, the publisher hides an event after 90 minutes without matching GPS evidence only while that exact route continues reporting. A single matching bus refreshes an already-confirmed event heartbeat. The active record remains available for monitoring and still needs affected-segment GPS traversal or explicit operator/admin action to clear.
...
- [ ] Confirmed detours without trustworthy rider geometry publish `alertVisible=true`, `riderVisible=false`, and no unsafe path or stop details
...
| `DETOUR_RIDER_VISIBILITY_MAX_EVIDENCE_AGE_MS` | `5400000` | Rich-detail visibility ceiling. The active record still requires GPS clear proof or operator action. |
