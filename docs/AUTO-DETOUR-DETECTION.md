...
- **Rider visibility** — confirmed detours stay rider-visible until GPS clear proof exists. The app must not hide a detour from age, `staleForReview`, no-current-vehicle state, or route-family activity. It hides only when the backend explicitly publishes `riderVisible: false` for safety reasons such as zero confirmed evidence, insufficient geometry, invalid geometry, stale mixed geometry evidence, or Route 400 stale sparse evidence. Stale mixed evidence stays backend-active but hidden with `riderVisibilityReason: "stale-mixed-evidence"` until current same-corridor evidence reconfirms a trustworthy path. Route 400 sparse evidence that spans more than 24 hours and has no current off-route vehicle is kept for backend monitoring but hidden with `riderVisibilityReason: "stale-sparse-evidence"` until same-day/current-vehicle evidence reconfirms it.
...
2. **Pass rollout health checks** — Confirm `/api/detour-rollout-health` reports `launchReadiness.status` as `pilot_ready` or `pilot_ready_with_cautions`. Treat unresolved baseline divergence as a launch blocker; pending auto-baseline updates should resolve after the stability recheck. Review stale warnings or any non-GPS clear history before enabling the rider UI.
