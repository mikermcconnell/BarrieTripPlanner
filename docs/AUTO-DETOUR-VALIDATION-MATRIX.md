...
| DET-002 | Likely path confidence gate | Keep an otherwise qualified confirmed alert active while hiding an unsafe likely path. A valid bounded event window may show `detailsPending`; uncertain stops remain non-notifying context and must never be promoted to skipped without evidence. | `api-proxy/__tests__/alertVisibility.test.js`, `api-proxy/__tests__/detourGeometry.test.js`, `src/__tests__/detourVisibility.test.js`, `src/__tests__/detourOverlays.test.js`, `src/__tests__/useAffectedStops.test.js` | QA sections 5, 6, 7 | Covered | Sparse evidence producing a convincing but wrong purple path or closed-stop warning |
...
| DET-005 | GPS-proven normal clearing | Clear only after normal-route traversal proof through the affected segment: either same-bus traversal or two unique same-route trips/vehicles collectively covering the clear window after latest off-route evidence. Default proof requires at least 75% coverage, required movement, a core-window sample, and an acceptable maximum progress gap. A stored threshold may be stricter but cannot lower the current floor. Persist measured/required values plus explicit pass flags, reject under-threshold proofs before enqueue, and revalidate them in the publisher before deletion after `clear-pending`. | `api-proxy/__tests__/detourV2Detector.test.js`, `api-proxy/__tests__/detourPublisher.test.js` | QA sections 10, 11 | Covered | Premature clearing from a weak traversal, forged/stale pass flags, or a stored threshold being treated as a ceiling instead of a floor |
| DET-006 | Stale active detour | Keep the backend record active until GPS clear proof or operator action. Detail visibility may age into `stale-evidence-awaiting-gps-clear`; once hidden for that reason, do not make it visible again from the same or older GPS timestamp. | `api-proxy/__tests__/staleClear.test.js`, `api-proxy/__tests__/detourPublisher.test.js` | QA sections 11, 12 | Covered | Old evidence staying visible too long, or stale publisher ticks reopening a hidden event without new evidence |
...
| DET-013 | Baseline safety | Block detection when the baseline is unsafe. Automatically accept stable route-level GTFS geometry changes after a force-refresh stability check only for routes without an active detour; active routes require operator review. | `api-proxy/__tests__/detourWorkerColdStart.test.js`, `api-proxy/__tests__/detourPublisher.test.js` | QA sections 1, 13, 14 | Covered locally; needs operations recheck | Treating an unstable or temporary live detour as normal service |
...
| DET-022 | Geometryless or detail-pending active detour | Keep the record for operations and never infer stops or a path from an official notice. A fresh confirmed event may set `alertVisible=true` and `detailsPending=true` only when its detector event window is valid and no more than 5km across; stale, invalid, or oversized windows stay hidden. Generic normal service does not clear it. | `api-proxy/__tests__/alertVisibility.test.js`, `api-proxy/__tests__/staleClear.test.js`, `api-proxy/__tests__/detourPublisher.test.js`, `src/__tests__/detourService.test.js`, `src/__tests__/detourAlertStrip.test.js` | QA sections 10, 11, 12 | Covered, policy corrected | Exposing an unbounded location, or hiding a fresh confirmed detour merely because exact details are pending |
...
| DET-025 | Rider visibility flapping | Rider-facing visibility must not toggle merely because a vehicle enters or leaves the current GTFS-RT snapshot. Public visibility still requires the normal evidence and geometry gates. Keep trusted geometry visible between buses while its evidence remains eligible; if it ages into `stale-evidence-awaiting-gps-clear`, keep it hidden until strictly newer GPS arrives. | `api-proxy/__tests__/staleClear.test.js`, `api-proxy/__tests__/detourPublisher.test.js`, `api-proxy/__tests__/detourV2Detector.test.js`, `src/__tests__/detourVisibility.test.js` | QA sections 2, 3, 4, 11 | Covered, needs live Route 8B/15B recheck after deploy | Preserving a weak mixed path versus hiding a real detour too early or reopening it from stale evidence |
...
| DET-028 | GTFS shape changed, baseline approval required | When public GTFS geometry meaningfully changes for a route with an active detour, keep the backend record and its previous visibility, mark `baselineReviewRequired`, and do not auto-adopt or clear it. A detour admin can approve the legitimate route change with `POST /api/baseline/routes`. Shape-ID-only churn with the same geometry should not trigger review. | `api-proxy/__tests__/detourPublisher.test.js`, `api-proxy/__tests__/detourWorkerColdStart.test.js` | QA sections 1, 13, 14 | Covered locally | Leaving a permanent redesign pending too long, or normalizing an active construction detour away |
...
| DET-036 | Terminal pull-in / layover loop | Preserve GTFS station/platform metadata. Do not treat a tiny major-terminal route-edge candidate as a rider detour without two independent complete departure/rejoin transitions, and do not mark a terminal platform skipped without two complete traces showing positive skip evidence. | `api-proxy/__tests__/detourV2Detector.test.js`, `api-proxy/__tests__/stopImpacts.test.js`, `api-proxy/__tests__/gtfsSnapshotStore.test.js` | QA sections 4, 5, 14 | Covered locally; needs live Allandale recheck | Terminal-only loops creating false short detours, or an overly broad safeguard hiding a strong non-terminal route-edge detour |
...
| DET-044 | GTFS baseline changes while active detour exists | Protect routes that already have active detours from automatic baseline adoption. Mark them `baselineReviewRequired`, retain the previous visibility decision, and require operator review instead of clearing with `baseline-auto-updated`. Routes without active detours still use the default 30-minute stable force-refresh path. | `api-proxy/__tests__/detourPublisher.test.js`, `api-proxy/__tests__/detourWorkerColdStart.test.js` | QA sections 1, 13, 14 | Covered locally; needs deployed operations recheck | Auto-accepting a construction detour as normal GTFS service, or permanently blocking a legitimate scheduled-route change from review |
...
| DET-073 | Stale unresolved records exposed as public alerts | Separate backend retention from rider-alert eligibility. Publish auto-detour alerts only with recent confirmed GPS evidence plus either meaningful alert geometry or a valid bounded event window; the latter is alert-only with `detailsPending=true`. Official notices may enrich a qualified event but never create or preserve it. | `api-proxy/__tests__/alertVisibility.test.js`, `api-proxy/__tests__/detourPublisher.test.js` | QA sections 2, 4, 5, 11, 14 | Covered, needs deployed Route 8A/8B/10/100 verification | Over-filtering low-frequency unannounced detours, exposing an oversized window, or allowing notice data to bypass auto-detection |
...
| DET-077 | Camera-neutral main-map selections | Detour, stop, route, address, official-impact, vehicle-cluster, and trip-preview selections must never move the main-map camera. Returning to the regular map also preserves the current viewport. Only dedicated camera controls such as My Location may change it. | `src/__tests__/homeMapNoAutomaticCamera.source.test.js`, `src/__tests__/detourViewportFreedom.source.test.js`, `src/__tests__/useMapNavigation.test.js` | QA sections 7, 8 | Covered, needs native gesture recheck | A future selection handler, state effect, or delayed fit reintroducing map snap-back |
...
| DET-080 | Active official detour disappears during low-frequency or holiday service | Keep a current MyRide detour notice visible as a separate official notice for its published date window, even when the auto-detour GPS record is stale and hidden. Do not create live geometry, skipped paths, or an `activeDetours` entry from the notice. | `src/__tests__/upcomingDetourNotices.test.js`, `src/__tests__/OfficialImpactStrip.test.js`, `src/__tests__/activeOfficialDetourWiring.source.test.js` | QA sections 5, 7, 11, 14 | Covered locally; needs native/web release verification | Incorrect notice dates or route extraction could hide a valid notice; stale GPS geometry must never be restored from notice data |
| DET-081 | Active construction detour is normalized away by a GTFS baseline update | Process the current GPS tick against the trusted baseline before evaluating adoption, then protect all routes active after that evidence. Stable GTFS changes on protected routes become review-required instead of auto-adopted. Explicitly reset resolved baseline-review flags on merge writes. | `api-proxy/__tests__/detourWorkerColdStart.test.js`, `api-proxy/__tests__/detourPublisher.test.js` | QA sections 1, 10, 13, 14 | Covered locally; needs Route 8B live recheck after deployment | A long-running legitimate route redesign needs timely operator baseline approval |
| DET-082 | Stale event flaps or clears from weak normal-route evidence | Once hidden as `stale-evidence-awaiting-gps-clear`, require a strictly newer GPS timestamp before restoring detail visibility. Accept a normal-route clear only when audited coverage, movement, core-sample, and progress-gap checks all pass, and make the publisher validate the measured values as well as the flags. | `api-proxy/__tests__/staleClear.test.js`, `api-proxy/__tests__/detourV2Detector.test.js`, `api-proxy/__tests__/detourPublisher.test.js` | QA sections 10, 11, 12, 14 | Covered locally; needs Route 8B live lifecycle recheck | A real detour can remain hidden between buses by design, but it must not be deleted without valid proof or reopened from stale evidence |
| DET-083 | Fresh confirmed detour has no exact path or affected-stop detail | Publish an alert-only event when GPS evidence is fresh and confirmed, geometry is missing or incomplete, and the event window is valid and at most 5km across. Known-invalid geometry never uses the fallback. Build cards from individual event documents before grouping so a pending same-route event stays separate from a detailed event. | `api-proxy/__tests__/alertVisibility.test.js`, `api-proxy/__tests__/detourPublisher.test.js`, `src/__tests__/detourAlertStrip.test.js`, `src/__tests__/detourService.test.js`, `src/__tests__/detourEvents.test.js` | QA sections 2, 5, 7, 11, 14 | Covered locally; needs Route 100 native/web recheck | Alert-only visibility must not be mistaken for exact map geometry or stop-closure proof |
| DET-084 | Broad route-edge geometry over-closes a major terminal | Retain GTFS `location_type` and `parent_station`, require two complete transition traces to activate a tiny major-terminal route-edge event, and require two time-ordered bypass traces that remain physically away from the platform before it becomes `skipped`. Otherwise publish it as boundary, served, or non-notifying `uncertain`. | `api-proxy/__tests__/gtfsSnapshotStore.test.js`, `api-proxy/__tests__/detourV2Detector.test.js`, `api-proxy/__tests__/stopImpacts.test.js`, `src/__tests__/useAffectedStops.test.js` | QA sections 4, 5, 6, 14 | Covered locally; needs Route 8 Lakeshore/Allandale recheck | A metadata-poor terminal can still depend on name recognition; a strong non-terminal route-edge detour must remain eligible |
| DET-085 | Low-frequency Route 8B details expire before the next bus reaches Blake | Freeze stale aging outside exact-route/exact-direction service. Count only distinct observed passenger trips that approach and pass the affected segment; one missed passage retains details and two hide them. Ignore scheduled-only, unobserved, cancelled, opposite-direction, and Route 8A trips. When passage cannot be measured, use `max(90 in-service minutes, 2 exact-direction headways + 10 minutes)`. Keep clearing GPS-proof-gated and require strictly newer detour GPS to re-show hidden details. | `api-proxy/__tests__/staleClear.test.js`, `api-proxy/__tests__/routeSchedule.test.js`, `api-proxy/__tests__/alertVisibility.test.js`, `api-proxy/__tests__/detourPublisher.test.js` | QA sections 10, 11, 12, 14 | Covered locally; needs live Sunday Route 8B lifecycle recheck | Ambiguous or incomplete schedule/shape data fails safe and may keep details visible for operator review; passage tracking must not replace normal-route clear proof |

## Recorded issues

### DET-085A — Route 8B Blake details were hidden before the first Sunday bus reached the segment

- Date reviewed: 2026-08-09
- Route: `8B`
- What happened: Sunday Route 8B service began around 9:11 AM, but production hid the still-active Blake event around 9:14 AM because its last matching GPS evidence was more than 90 wall-clock minutes old. Overnight time and a Route 8B bus elsewhere on the route were treated as enough for stale detail suppression even though no Sunday trip had reached Blake.
- Fix:
  - exclude overnight and all other exact-route/exact-direction non-service time
  - track only distinct observed passenger trips whose scheduled shape/direction matches the event and whose GPS progress approaches and passes the affected segment
  - retain details after the first missed passage and hide after two distinct missed passages
  - ignore scheduled-only, unobserved, cancelled, opposite-direction, and sibling-route trips
  - when reliable passage cannot be measured, use `max(90 active-service minutes, 2 exact-direction headways + 10 minutes)`
  - keep the active event and official notice separate from visibility suppression; keep normal-route clearing on the existing audited GPS proof path
  - require strictly newer matching detour GPS to restore details after stale suppression
- Tests: `api-proxy/__tests__/staleClear.test.js`, `api-proxy/__tests__/routeSchedule.test.js`, `api-proxy/__tests__/alertVisibility.test.js`, `api-proxy/__tests__/detourPublisher.test.js`
- Remaining risk: production needs a Sunday/low-frequency lifecycle observation confirming the event stays public before Blake passage, logs one then two genuine missed opportunities correctly, and still clears only from valid normal-route GPS proof.
...
- Docs updated:
  ...
  - this matrix
- Superseded 2026-08-09 by DET-085: stale visibility now uses observed passage opportunities or an active-service-time fallback instead of a fixed wall-clock window.
...
- What should have happened: map selections should change content only. The rider's current center and zoom should remain untouched unless they press a dedicated camera control such as My Location.
- Classification:
  - frontend rendering / map interaction
- Root cause:
  - Android device diagnostics on `1.0.8` build `21` recorded one explicit JavaScript camera command when Route `8B` was selected, followed by repeated returns to the exact detour center with no second JavaScript command
  - this ruled out repeated app effects and timers; MapLibre React Native `10.4.2` has a matching upstream Android gesture snap-back defect after programmatic camera changes (`maplibre-react-native#1464`)
  - a native stale-stop replay mitigation did not eliminate the rider-reported behavior, so selection-driven camera movement is not considered reliable on the supported Android stack
- Fix:
  - remove automatic camera movement from main-map detour, stop, address, official-impact, route, vehicle-cluster, and trip-preview selections on native and web
  - keep detour and regular-view mode changes camera-neutral
  - retain only dedicated rider-requested camera actions such as My Location and development-only map controls
- Tests added/updated:
  - `src/__tests__/detourViewportFreedom.source.test.js`
  - `src/__tests__/homeMapNoAutomaticCamera.source.test.js`
  - `src/__tests__/useMapNavigation.test.js`
- Docs updated:
  - `AUTO-DETOUR-DETECTION.md`
  - `AUTO-DETOUR-QA-CHECKLIST.md`
  - this matrix
- Earlier web verification remains historical evidence for the superseded one-shot approach; the current acceptance criterion is zero selection-driven camera commands.
- Remaining risk: manually confirm on Android build `24` that detour, stop, route, address, vehicle-cluster, and trip-preview selections never change the viewport.
