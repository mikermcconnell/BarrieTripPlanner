...
| DET-063 | Confirmed normal terminal circulation | Operator-confirmed terminal paths must not contribute off-route evidence when the trip begins/ends at the configured terminal stop, route progress remains inside the configured edge limit, and GPS matches the approved circulation polyline. Eligibility must come from GTFS trip terminals rather than route IDs; a materially different path remains detour evidence. | `api-proxy/__tests__/detourV2Detector.test.js`, `api-proxy/__tests__/detourRouteConfig.test.js`, `api-proxy/__tests__/gtfsLoader.test.js` | QA sections 4, 5, 14 | Covered, needs deployed Route 8A/100 recheck | A terminal circulation pattern changing beyond its configured corridor, or a real closure following the exact same approved movement |
...
| DET-077 | One-time detour and regular-map camera handoff | Opening the banner for multiple detours must not move the map. Selecting a specific detour fits only that event's geometry once. Returning to the regular map preserves the current viewport. Rider pan and pinch-zoom must retain camera control without delayed or repeated refocusing. | `src/__tests__/detourAlertStrip.test.js`, `src/__tests__/detourViewportFreedom.source.test.js`, `src/__tests__/detourViewport.test.js` | QA sections 7, 8 | Covered, needs native gesture recheck | A future state-driven camera effect or delayed fit reintroducing map snap-back |
...
- Fix:
  - keep detour focusing attached only to the explicit initial detour selection and fit only the selected event's candidate segments
...
- Tests added/updated:
  ...
  - `src/__tests__/detourAlertStrip.test.js`
- Docs updated:
  - `AUTO-DETOUR-DETECTION.md`
  - `AUTO-DETOUR-QA-CHECKLIST.md`
  - this matrix
- Web verification 2026-08-08: with the `saunders-welham` dev fixture, selecting the active detour issued one fit action. Keyboard panning changed the map center, and no later fit action occurred during the following eight seconds or passive realtime updates.
