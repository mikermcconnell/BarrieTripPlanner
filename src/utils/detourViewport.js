...
function normalizeFocusedRouteIds({ activeDetours, focusedRouteId, focusedRouteIds }) {
  ...
  // An explicit but no-longer-active selection must not zoom to every other
  // active detour in the city.
  return ids.length > 0 ? normalized : Object.keys(activeDetours || {});
}
...
function getViewportSegments(detour, segmentIndex = null) {
  ...
  return segments[segmentIndex] ? [segments[segmentIndex]] : [];
}
