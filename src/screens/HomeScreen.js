...
import { HOME_MAP_THEME, getAllRoutesOpacity } from '../config/homeMapTheme';
...
import { createMapPerformanceMonitor } from '../utils/mapPerformanceMonitor';
...
import MapBottomControlTray from '../components/home-map/MapBottomControlTray';
import HomeMapVehicleLayer from '../components/home-map/HomeMapVehicleLayer';
import VehicleQuickCard from '../components/home-map/VehicleQuickCard';
import VehicleClusterQuickCard from '../components/home-map/VehicleClusterQuickCard';

const HOME_ROUTE_LAYER_BELOW_ID = Platform.OS === 'android'
  ? HOME_MAP_VEHICLE_LAYER_ANCHOR_ID
  : undefined;
...
import TransitStatusTray from '../components/home-map/TransitStatusTray';
...
const SearchIcon = ({ size = 20, color = COLORS.textSecondary }) => (
  <PrimaryIcon name="Search" size={size} color={color} />
);
...
const MAP_LAYER_INDEX = {
  ...
  REGULAR_STOPS_CENTER: 702,
  REGULAR_STOPS_LABEL: 703,
  TRIP_ROUTES: 760,
...
const MAP_GESTURE_RELEASE_TIMEOUT_MS = 1500;
// Route/label visuals use broad zoom tiers. Half-step updates avoid rebuilding
// every MapLibre source several times during a single pinch gesture.
const MAP_ZOOM_RENDER_STEP = 0.5;
...
const getBaseRouteVisual = ({ shape, currentZoom }) => ({
  routeOpacity: getAllRoutesOpacity(currentZoom),
  routeStrokeWidth:
...
const HomeMapRoutesLayer = React.memo(({
  ...
  const routeFeatureCollections = buildHomeRouteFeatureCollections(displayedShapes.flatMap((shape) => {
    ...
    const layerOrder = getDetourRouteLayerOrder({
    ...
    if (!shouldRenderShape) {
      return shouldKeepHiddenRouteShapeLayerMounted({
        routeId: shape.routeId,
        activeDetourRouteIds,
        isDetourView,
      }) ? [{
    ...
    return visibleRouteSegments.map((coordinates, segmentIndex) => ({
      id: visibleRouteSegments.length === 1
        ? shape.id
        : `${shape.id}-visible-${segmentIndex}`,
      routeId: shape.routeId,
      coordinates: getCartographicRouteCoordinates(coordinates, { zoom: currentZoom }),
      layerOrder,
      routeColor: routeVisual.routeColor,
      routeOpacity: routeVisual.routeOpacity,
      routeStrokeWidth: routeVisual.routeStrokeWidth,
    }));
  }));

  return (
    <>
      {HOME_ROUTE_LAYER_ORDERS.map((layerOrder) => {
        const sourceId = getHomeRouteSourceId(layerOrder);
        return (
          <MapLibreGL.ShapeSource
            key={sourceId}
            id={sourceId}
            shape={routeFeatureCollections[layerOrder]}
          >
            <MapLibreGL.LineLayer
              id={`${sourceId}-lines`}
              layerIndex={layerOrder + 1}
              belowLayerID={HOME_ROUTE_LAYER_BELOW_ID}
              style={{
                lineColor: ['get', 'routeColor'],
                lineWidth: ['get', 'routeStrokeWidth'],
  ...
  return (
    ...
    >
      <MapLibreGL.CircleLayer
        ...
        belowLayerID={HOME_MAP_VEHICLE_LAYER_ANCHOR_ID}
        style={{
          circleRadius: ['case', ['==', ['get', 'isSelected'], 1], 9, 6],
          circleColor: [
            'case',
            ['==', ['get', 'isSelected'], 1],
            MAP_MARKER_THEME.stopSelected,
            ['==', ['get', 'isClosed'], 1],
            MAP_MARKER_THEME.closedStop,
            MAP_MARKER_THEME.stopOutline,
          ],
        }}
      />
      <MapLibreGL.CircleLayer
      ...
      />
      <MapLibreGL.CircleLayer
        id="home-stops-center"
        layerIndex={MAP_LAYER_INDEX.REGULAR_STOPS_CENTER}
        aboveLayerID="home-stops-fill"
        style={{
          circleRadius: ['case', ['==', ['get', 'isSelected'], 1], 2.25, 1.5],
          circleColor: [
            'case',
            ['==', ['get', 'isSelected'], 1],
            MAP_MARKER_THEME.stopSelected,
      ...
      <MapLibreGL.SymbolLayer
        id="home-stops-code-labels"
        layerIndex={MAP_LAYER_INDEX.REGULAR_STOPS_LABEL}
        aboveLayerID="home-stops-center"
        minZoomLevel={MAP_MARKER_THEME.stopCodeMinZoom}
        style={{
          textField: ['get', 'stopCode'],
          textSize: 11,
          textFont: ['Noto Sans Bold'],
          textColor: MAP_MARKER_THEME.stopOutline,
...
const HomeMapTopStopsOverlay = React.memo(({
  ...
  const isClosedStopMarker = (stop) => Boolean(stop?.isClosed || stop?.isRouteScopedClosure);
  const visibleStops = displayedStops
    .filter((stop) => Number.isFinite(stop?.latitude) && Number.isFinite(stop?.longitude))
    .filter((stop) => showAllStopMarkers || isClosedStopMarker(stop));
  const stopsById = new Map(visibleStops.map((stop) => [String(stop.id), stop]));
  const stopFeatures = {
    type: 'FeatureCollection',
    features: visibleStops.map((stop, index) => {
      const isClosed = isClosedStopMarker(stop);
      const stopCode = String(stop.code ?? stop.stopCode ?? stop.id ?? '');
      return {
        type: 'Feature',
        id: String(stop.id ?? `top-stop-${index}`),
        geometry: { type: 'Point', coordinates: [stop.longitude, stop.latitude] },
        properties: {
          id: String(stop.id ?? `top-stop-${index}`),
          isClosed: isClosed ? 1 : 0,
          isSelected: selectedStopId != null && stop.id === selectedStopId ? 1 : 0,
          opacity: isClosed ? closedStopOpacity : 1,
          showStopCode: isClosed && (isDetourView || hasDetourFocus) && stopCode ? 1 : 0,
          stopCode,
        },
      };
    }),
  };
  const handleTopStopPress = (event) => {
    const stopId = event?.features?.[0]?.properties?.id;
    const stop = stopId != null ? stopsById.get(String(stopId)) : null;
    if (stop) onStopPress?.(stop);
  };

  return (
    <MapLibreGL.ShapeSource
      id="home-top-stops-source"
      shape={stopFeatures}
      hitbox={{ width: 44, height: 44 }}
      onPress={handleTopStopPress}
    >
      <MapLibreGL.CircleLayer
        id="home-top-stops-open-outer"
        filter={['==', ['get', 'isClosed'], 0]}
        belowLayerID={HOME_MAP_VEHICLE_LAYER_ANCHOR_ID}
        style={{
          circleRadius: ['case', ['==', ['get', 'isSelected'], 1], 11, 8],
          circleColor: COLORS.white,
          circleStrokeColor: ['case', ['==', ['get', 'isSelected'], 1], MAP_MARKER_THEME.stopSelected, MAP_MARKER_THEME.stopOutline],
          circleStrokeWidth: 2,
        }}
      />
      <MapLibreGL.CircleLayer
        id="home-top-stops-open-inner"
        aboveLayerID="home-top-stops-open-outer"
        filter={['==', ['get', 'isClosed'], 0]}
        style={{
          circleRadius: ['case', ['==', ['get', 'isSelected'], 1], 4.5, 3],
          circleColor: ['case', ['==', ['get', 'isSelected'], 1], MAP_MARKER_THEME.stopSelected, MAP_MARKER_THEME.stopOutline],
        }}
      />
      <MapLibreGL.CircleLayer
        id="home-top-stops-closed-outer"
        aboveLayerID="home-top-stops-open-inner"
        filter={['==', ['get', 'isClosed'], 1]}
        style={{
          circleRadius: ['case', ['==', ['get', 'isSelected'], 1], 7, 5.5],
          circleColor: COLORS.white,
          circleStrokeColor: ['case', ['==', ['get', 'isSelected'], 1], MAP_MARKER_THEME.stopSelected, MAP_MARKER_THEME.closedStop],
          circleStrokeWidth: 1.5,
        }}
      />
      <MapLibreGL.CircleLayer
        id="home-top-stops-closed-dot"
        aboveLayerID="home-top-stops-closed-outer"
        filter={['==', ['get', 'isClosed'], 1]}
        style={{
          circleRadius: 1.75,
          circleColor: MAP_MARKER_THEME.closedStop,
        }}
      />
      <MapLibreGL.SymbolLayer
        id="home-top-stops-closed-label"
        aboveLayerID="home-top-stops-closed-dot"
        filter={['==', ['get', 'showStopCode'], 1]}
        style={{
          textField: ['get', 'stopCode'],
          textSize: 10,
          textColor: MAP_MARKER_THEME.closedStop,
          textHaloColor: COLORS.white,
          textHaloWidth: 2,
          textOffset: [2.2, -1.1],
          textAllowOverlap: true,
...
const AndroidLiveBusMarker = React.memo(({
  ...
  const sourceCoordinate = vehicle?.coordinate || {};
  const { latitude, longitude, bearing } = useAnimatedBusPosition(vehicle, { snapPath: null });
  const markerWidth = routeLabel.length >= 3 ? 42 : routeLabel.length === 2 ? 36 : 30;
  const animatedCoordinate = normalizeMapCoordinate({ latitude, longitude });

  if (!normalizeMapCoordinate(sourceCoordinate) || !animatedCoordinate) {
    return null;
  }
  ...
  return (
    <MapLibreGL.MarkerView
      ...
      coordinate={[animatedCoordinate.longitude, animatedCoordinate.latitude]}
      anchor={{ x: 0.5, y: 0.5 }}
      pointerEvents="none"
    >
      <View
...
const HomeMapVehiclesLayer = React.memo(({
  ...
  // Keep the always-on home fleet in one MapLibre source. Resolve each route path
  // once per GPS snapshot so the animation can follow corners without doing route
  // searches on every animation frame.
  if (Platform.OS === 'android') {
    return (
      <HomeMapVehicleLayer
        ...
        onSelectVehicleCluster={onSelectVehicleCluster}
...
const sanitizeTripPreviewLines = (lines) => (
  Array.isArray(lines)
    ? lines
        .map((line) => {
          ...
          return {
            ...line,
            coordinates,
            labelCoordinate: normalizeMapCoordinate(line?.labelCoordinate),
          };
        })
        .filter(Boolean)
    : []
...
const sanitizeTripPreviewMarkers = (markers) => (
  Array.isArray(markers)
    ? markers
        .map((marker) => {
          const coordinate = normalizeMapCoordinate(marker?.coordinate);
          return coordinate ? { ...marker, coordinate } : null;
        })
        .filter(Boolean)
    : []
);
...
const HomeMapTripPreviewLayer = React.memo(({
  ...
  const safeMapData = useMemo(() => ({
    tripRoutes: sanitizeTripPreviewLines(tripRouteCoordinates),
    endpointMarkers: sanitizeTripPreviewMarkers(tripEndpointMarkers),
    approachLines: sanitizeTripPreviewLines(busApproachLines),
    intermediateMarkers: sanitizeTripPreviewMarkers(intermediateStopMarkers),
    stopMarkers: sanitizeTripPreviewMarkers(tripMarkers),
    boardingMarkers: sanitizeTripPreviewMarkers(boardingAlightingMarkers),
    transfers: sanitizeTripPreviewMarkers(transferMarkers),
    vehicles: Array.isArray(tripVehicles)
      ? tripVehicles
          .map((vehicle) => {
            const coordinate = normalizeMapCoordinate(vehicle?.coordinate);
            return coordinate ? { ...vehicle, coordinate } : null;
          })
          .filter(Boolean)
      : [],
  }), [
  ...
  return (
    <>
    {safeMapData.tripRoutes.map((tripRoute) => (
      <RoutePolyline
        key={tripRoute.id}
    ...
    {safeMapData.endpointMarkers.map((marker) => (
    ...
    {safeMapData.approachLines.map((line) => (
      <RoutePolyline
        key={line.id}
        id={line.id}
    ...
    {safeMapData.intermediateMarkers.map((marker) => (
      <MapLibreGL.PointAnnotation
        key={marker.id}
    ...
    {safeMapData.stopMarkers.map((marker) => (
      <MapLibreGL.MarkerView
        key={marker.id}
        coordinate={[marker.coordinate.longitude, marker.coordinate.latitude]}
        pointerEvents="none"
    ...
    {safeMapData.boardingMarkers.map((marker) => (
      <MapLibreGL.MarkerView
        key={marker.id}
        id={`ba-${marker.id}`}
        coordinate={[marker.coordinate.longitude, marker.coordinate.latitude]}
    ...
    {safeMapData.transfers.map((marker) => (
      <MapLibreGL.MarkerView
        key={marker.id}
    ...
    {safeMapData.vehicles.map((vehicle) => {
...
const HomeMapView = React.memo(({
  ...
  displayedStops,
  selectedStopId,
  hasClosedStopsForDisplay,
  closedStopMarkerOpacity,
  stopsGeoJson,
  handleStopLayerPress,
  displayedVehicles,
  getRouteColor,
  ...
  >
    ...
    <HomeMapTopStopsOverlay
      isTripPreviewMode={isTripPreviewMode}
      visible={isDetourView || hasDetourFocus || hasClosedStopsForDisplay}
      displayedStops={displayedStops}
      selectedStopId={selectedStopId}
      onStopPress={handleDetourStopPress}
      showAllStopMarkers={isDetourView || hasDetourFocus}
      closedStopOpacity={closedStopMarkerOpacity}
      isDetourView={isDetourView}
      hasDetourFocus={hasDetourFocus}
    />

    {!isTripPreviewMode && shouldShowDetourGeometryOverlay({ isDetourView, hasDetourFocus }) && detourOverlays.map((overlay) => (
      <DetourOverlay
    ...
    {/* Render live buses last; stop layers are also anchored below them so the
        map hierarchy stays buses, stops, then route lines on Android. */}
    ...
    {!isTripPreviewMode && (
      <BusHubOverlay
        currentZoom={currentZoom}
        aboveLayerID={HOME_MAP_VEHICLE_TOP_LAYER_ID}
      />
    )}
  </MapLibreGL.MapView>
), (prev, next) => (
  ...
  prev.handleRegionIsChanging === next.handleRegionIsChanging &&
  ...
  prev.hasClosedStopsForDisplay === next.hasClosedStopsForDisplay &&
  prev.closedStopMarkerOpacity === next.closedStopMarkerOpacity &&
  ...
  prev.onSelectVehicle === next.onSelectVehicle &&
  prev.onSelectVehicleCluster === next.onSelectVehicleCluster &&
  prev.feedIsStale === next.feedIsStale &&
  prev.animationActive === next.animationActive
));
...
const HomeScreen = ({ route }) => {
  ...
  const mapPerformanceMonitorRef = useRef(PERF_DEBUG ? createMapPerformanceMonitor({
    onSample: (sample) => logger.info('[perf][home-map] %s', JSON.stringify(sample)),
  }) : null);
  ...
  const [selectedVehicleClusterIds, setSelectedVehicleClusterIds] = useState([]);
  ...
  useEffect(() => () => {
    ...
    if (mapGestureReleaseTimerRef.current) {
      clearTimeout(mapGestureReleaseTimerRef.current);
      mapGestureReleaseTimerRef.current = null;
    }
  }, []);
  ...
  }, [rawHandleRouteSelect]);

  const handleRouteFamilySelect = useCallback((routeIds = []) => {
    const familyRouteIds = Array.isArray(routeIds) ? routeIds.filter(Boolean) : [];
    const familyAlreadySelected =
      familyRouteIds.length > 0 &&
      familyRouteIds.every((routeId) => selectedRoutes.has(routeId)) &&
      selectedRoutes.size === familyRouteIds.length;

    selectRoutes(familyAlreadySelected ? [] : familyRouteIds);
  }, [selectRoutes, selectedRoutes]);
  ...
  const shouldShowDetourModeExit = !isTripPlanningMode && isDetourView;

  const returnToRegularMapView = useCallback(() => {
    setShowRoutes(true);
    handleMapViewModeChange('regular');
    // A content-mode change must never reclaim the camera. Preserve the
    // rider's exact center and zoom until another explicit focus action.
  }, [handleMapViewModeChange]);

  const handleMapViewModeControlChange = useCallback((nextMode) => {
    if (nextMode === 'regular') {
      returnToRegularMapView();
      return;
    }

    handleMapViewModeChange(nextMode);
  }, [handleMapViewModeChange, returnToRegularMapView]);
  ...
  const handleDevMapPan = useCallback((direction) => {
    ...
    recordMapCameraDiagnostic('camera.command', {
      source: 'home.dev-pan',
      direction,
      region: nextRegion,
    });
  ...
  const selectedVehicle = useMemo(
    () => findVehicleById(mapDisplayedVehicles, selectedVehicleId),
    [mapDisplayedVehicles, selectedVehicleId]
  );
  useEffect(() => {
    if (!selectedVehicle?.tripId) {
      setSelectedVehicleTripUpdate(null);
      return undefined;
    }
    ...
    const loadTripUpdate = async () => {
      ...
      try {
        ...
        logger.warn('Could not load selected bus schedule status:', error);
        if (active && requestId === latestRequest) setSelectedVehicleTripUpdate(null);
      }
    };

    void loadTripUpdate();
    const refreshInterval = setInterval(() => void loadTripUpdate(), 30000);
    return () => {
      active = false;
  ...
  }, [mapDisplayedVehicles, selectedVehicleClusterIds]);
  const vehicleFeedIsStale = Boolean(
    diagnostics?.realtimeVehicles?.isStale ||
    diagnostics?.realtimeVehicles?.isFeedStale ||
    diagnostics?.realtimeVehicles?.reason === 'gps_feed_stale' ||
    diagnostics?.realtimeVehicles?.status === 'error'
  );
  ...
  const handleSelectVehicle = useCallback((vehicleId) => {
    ...
    setSelectedVehicleClusterIds([]);
    setSelectedVehicleId(vehicleId);
  }, []);

  const handleSelectVehicleCluster = useCallback((vehicleIds) => {
    suppressNextMapTapRef.current = true;
    setSelectedStop(null);
    setSelectedVehicleId(null);
    setSelectedVehicleClusterIds(vehicleIds);
  }, []);
  ...
  const showDetourRouteOnMap = useCallback((routeId, detourEvent = detourSheetEvent) => {
    ...
    handleMapViewModeChange('detour');
  }, [detourSheetEvent, handleMapViewModeChange]);
  ...
  const handleDetourOverlayPress = useCallback((routeId, _segment = null, segmentIndex = null, overlay = null) => {
    if (!routeId) return;
    recordMapCameraDiagnostic('selection.detour-overlay', {
      routeId,
      segmentIndex,
      cameraNeutral: true,
    });
  ...
  const toggleBusStops = useCallback(() => {
    setShowStops((visible) => !visible);
  }, []);
  ...
  const primaryNoticeVisibility = getHomeNoticeVisibility({
    hasActiveDetour: shouldShowDetourStatusRow,
    officialImpactCount: visibleOfficialServiceImpacts.length,
    upcomingDetourCount: visibleUpcomingDetourNotices.length,
    hasHolidayNotice: Boolean(visibleHomeHolidayServiceInfo),
  });
  const showPrimaryActiveDetourNotice = primaryNoticeVisibility.activeDetour;
  const showPrimaryOfficialNotice = primaryNoticeVisibility.official;
  const showPrimaryUpcomingNotice = primaryNoticeVisibility.upcoming;
  const showPrimaryHolidayNotice = primaryNoticeVisibility.holiday;
  const shouldShowDetourChrome = !isTripPlanningMode && (canUseDetourView || visibleOfficialServiceImpacts.length > 0);
  const closedStopMarkerOpacity = 1;
  ...
  const stopsGeoJson = useMemo(() => ({
    ...
    features: mapStopsForDisplay.map((stop) => ({
      ...
      properties: {
        ...
        isClosed: stop.isClosed || stop.isRouteScopedClosure ? 1 : 0,
        stopCode: String(stop.code ?? stop.stopCode ?? stop.id ?? ''),
      },
    })),
  ...
  const handleStopLayerPress = useCallback((event) => {
    ...
    if (stop) {
      recordMapCameraDiagnostic('selection.stop-marker', {
  ...
  const stopFollowingUserLocation = useCallback(() => {
    recordMapCameraDiagnostic('camera.command', {
      source: 'home.stop-following-user-location',
      followUserLocation: false,
    });
    cameraRef.current?.setCamera({
      followUserLocation: false,
  ...
  const cancelPendingLocationCenter = useCallback(() => {
    // Avoid issuing an imperative Camera command at the start of every drag or
    // pinch. That command can interrupt MapLibre's Android gesture recognizer.
    if (!hasActiveLocationCenterRequest({
      isCentering: isCenteringOnUserLocation,
  ...
  }, [isCenteringOnUserLocation, stopFollowingUserLocation]);

  const setMapGestureActivity = useCallback((active) => {
    if (mapGestureReleaseTimerRef.current) {
      clearTimeout(mapGestureReleaseTimerRef.current);
      mapGestureReleaseTimerRef.current = null;
    }
    ...
    if (active) {
      // RegionDidChange should release this after the finger lifts. The timer is
      // a safety net for interrupted native gesture sequences.
      mapGestureReleaseTimerRef.current = setTimeout(() => {
        mapGestureReleaseTimerRef.current = null;
        setIsMapGestureActive(false);
      }, MAP_GESTURE_RELEASE_TIMEOUT_MS);
    }
  }, []);

  const handleRegionWillChange = useCallback((feature) => {
    recordMapCameraDiagnostic('map.region.will-change', {
      isUserInteraction: Boolean(feature?.properties?.isUserInteraction),
      center: feature?.geometry?.coordinates || null,
      zoomLevel: feature?.properties?.zoomLevel ?? null,
      selectedStopId: selectedStop?.id ?? null,
      detourRouteId: detourSheetRouteId,
    });
    if (feature?.properties?.isUserInteraction) {
      mapPerformanceMonitorRef.current?.onGestureStart();
      setMapGestureActivity(true);
      cancelPendingLocationCenter();
    }
  }, [cancelPendingLocationCenter, detourSheetRouteId, selectedStop?.id, setMapGestureActivity]);

  const handleRegionIsChanging = useCallback((feature) => {
    if (feature?.properties?.isUserInteraction) {
      mapPerformanceMonitorRef.current?.onRegionChanging();
    }
  }, []);

  const handleMapFrameStart = useCallback(() => {
    mapPerformanceMonitorRef.current?.onFrameStart();
  }, []);

  const handleMapFrameEnd = useCallback(() => {
    mapPerformanceMonitorRef.current?.onFrameEnd();
  }, []);
  ...
  const handleRegionChange = useCallback((feature) => {
    ...
    recordMapCameraDiagnostic('map.region.did-change', {
      isUserInteraction: Boolean(properties.isUserInteraction),
      center: [lng, lat],
      zoomLevel: zoom,
      selectedStopId: selectedStop?.id ?? null,
      detourRouteId: detourSheetRouteId,
    ...
    <View style={styles.container}>
      {mapReadyToMount ? (
        <HomeMapView
          ...
          handleRegionIsChanging={handleRegionIsChanging}
          ...
          selectedStopId={selectedStop?.id ?? null}
          hasClosedStopsForDisplay={hasClosedStopsForDisplay}
          closedStopMarkerOpacity={closedStopMarkerOpacity}
          stopsGeoJson={stopsGeoJson}
          handleStopLayerPress={handleStopLayerPress}
          ...
          onSelectVehicleCluster={handleSelectVehicleCluster}
          feedIsStale={vehicleFeedIsStale}
          animationActive={isFocused && !isMapGestureActive}
        />
      ) : (
      ...
      {!isTripPlanningMode && showPrimaryHolidayNotice && !shouldShowDetourChrome && (
      ...
      {shouldShowDetourChrome && (
        <View style={styles.detourStatusStack}>
          ...
          {shouldShowDetourModeExit && (
            <View style={styles.detourModeExitRow}>
              <TouchableOpacity
                style={styles.detourModeExitButton}
                activeOpacity={0.82}
                onPress={returnToRegularMapView}
                accessibilityRole="button"
                accessibilityLabel="Back to Regular Route View"
              >
                <Icon name="Map" size={16} color={COLORS.primaryDark} />
                <Text style={styles.detourModeExitText}>Back to Regular Route View</Text>
                <Text style={styles.detourModeExitSubtext}>Show all routes</Text>
              </TouchableOpacity>
            </View>
          )}
          {showPrimaryUpcomingNotice && (
      ...
      {/* Bottom map controls: one route/location tray. */}
      {showMainMapFloatingControls && !isDetourView && (
        <>
          {!selectedVehicle && selectedVehicleCluster.length <= 1 && !mapTapLocation && (
            <DetourExplainerButton
              style={{
                position: 'absolute',
                right: HOME_MAP_THEME.sideMargin,
                bottom:
                  HOME_MAP_CHROME_OFFSETS.mapUtilityBottom +
                  floatingBottomOffset +
                  HOME_MAP_THEME.bottomTrayMinHeight +
                  SPACING.sm,
                zIndex: 1000,
              }}
            />
          )}
          <MapBottomControlTray
            style={{ bottom: HOME_MAP_CHROME_OFFSETS.mapUtilityBottom + floatingBottomOffset }}
            routes={routes}
            selectedRoutes={selectedRoutes}
            onRouteSelect={handleRouteSelect}
            onRouteFamilySelect={handleRouteFamilySelect}
            getRouteColor={getRouteColor}
            isRouteDetouring={isRouteDetouring}
            showStops={showStops}
            onToggleStops={toggleBusStops}
            onCenterOnLocation={centerOnUserLocationOnce}
            isCenteringOnLocation={isCenteringOnUserLocation}
          />
        </>
      )}

      {!isTripPlanningMode && selectedVehicle && (
        <VehicleQuickCard
          vehicle={selectedVehicle}
          routeLabel={String(getRouteLabel(selectedVehicle) || selectedVehicle.routeId || '?')}
          routeColor={getRouteColor(selectedVehicle.routeId)}
          feedIsStale={vehicleFeedIsStale}
          stops={stops}
          routeStopsMapping={routeStopsMapping}
          routeStopSequencesMapping={routeStopSequencesMapping}
          tripUpdate={selectedVehicleTripUpdate}
      ...
      {!isTripPlanningMode && selectedVehicleCluster.length > 1 && (
        <VehicleClusterQuickCard
          vehicles={selectedVehicleCluster}
          getRouteLabel={getRouteLabel}
          getRouteColor={getRouteColor}
          onSelectVehicle={handleSelectVehicle}
          onClose={() => setSelectedVehicleClusterIds([])}
          style={{
            bottom:
              HOME_MAP_CHROME_OFFSETS.mapUtilityBottom +
              floatingBottomOffset +
          ...
          {!selectedStop && (
            <SheetErrorBoundary fallbackMessage="Trip results failed to load.">
              <TripBottomSheet
                ...
                isLoading={isTripLoading}
                isRefining={tripState.isRefining}
                loadingMessage={tripState.loadingMessage}
                error={tripError}
                hasSearched={hasTripSearched}
...
const HOME_MAP_CHROME_OFFSETS = Object.freeze({
  detourStatusTop: 84,
  mapUtilityBottom: 16,
});
...
const styles = StyleSheet.create({
  ...
  searchBar: {
    position: 'absolute',
    top: HOME_MAP_THEME.topMargin + STATUS_BAR_OFFSET,
  ...
  whereToAutocomplete: {
    ...
    minHeight: HOME_MAP_THEME.searchHeight,
    borderRadius: HOME_MAP_THEME.searchRadius,
  ...
  detourStatusStack: {
    position: 'absolute',
    top: HOME_MAP_CHROME_OFFSETS.detourStatusTop + STATUS_BAR_OFFSET,
    left: SPACING.sm,
    right: SPACING.sm,
    zIndex: 997,
    gap: SPACING.xs,
  ...
  detourLegendWithUpcoming: {
    top: 246 + STATUS_BAR_OFFSET,
  },
  detourLegendWithCollapsedUpcoming: {
    top: 140 + STATUS_BAR_OFFSET,
  },
  statusBadgeLive: {
    flexDirection: 'row',
    alignItems: 'center',
