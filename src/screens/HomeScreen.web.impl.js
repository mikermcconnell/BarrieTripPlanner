...
import BusHubOverlay from '../components/BusHubOverlay.web';
...
const HomeScreen = ({ route }) => {
  ...
  const selectedVehicle = useMemo(
    () => mapDisplayedVehicles.find((vehicle) => String(vehicle.id) === String(selectedVehicleId)) || null,
  ...
  const handleSaveCurrentTrip = useCallback(async () => {
    ...
    if (sharedTripEdit?.shareId) {
      ...
      if (!sharedResult.success) {
        showMessage('Could not update shared trip', sharedResult.error || 'Open the latest shared trip and try again.');
        return;
      }
      await addSavedTrip(payload);
      setSharedTripEdit(null);
      showMessage('Shared trip updated', 'Everyone with the link will see this version.');
  ...
  useEffect(() => {
    ...
    if (mapStopsForDisplay.length > PERFORMANCE_BUDGETS.MAP_MAX_VISIBLE_STOPS) {
      logger.warn(
        '[perf][home-map-web] Visible stops=%d (budget=%d)',
        mapStopsForDisplay.length,
        PERFORMANCE_BUDGETS.MAP_MAX_VISIBLE_STOPS
      );
      perfRef.current.lastWarnTs = nowTs;
    }
  }, [displayedVehicles.length, mapStopsForDisplay.length]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!isTripPreviewMode && mapStopsForDisplay.map((stop) => (
          <WebStopMarker
            key={stop.id}
            stop={stop}
            ...
            showStopCode={currentZoom >= MAP_MARKER_THEME.stopCodeMinZoom}
          />
        ))}
        ...
        {!isTripPreviewMode && mapDisplayedVehicles.map((vehicle) => {
          ...
          const quickDetails = getVehicleQuickDetails({
            vehicle,
            stops,
            routeStopsMapping,
            routeStopSequencesMapping,
            tripUpdate: String(vehicle.id) === String(selectedVehicleId)
