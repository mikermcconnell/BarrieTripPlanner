...
- Android bottom UI gotcha: the system navigation bar can cover bottom-aligned content. Any full-screen loading, modal, map, or bottom action UI must include safe bottom spacing using `src/utils/androidNavigationBar.js` helpers such as `useSafeBottomInset`, `useSafeBottomPadding`, or `useAndroidBottomChromeLift`.
